...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-bold capitalize shadow-sm ${config[status] || "bg-gray-100 text-gray-600 border-gray-200"}`}>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center rounded-3xl border-2 border-dashed border-gray-200 p-20 text-center">
      ...
      <p className="mt-1 text-gray-500 text-sm">When you buy fresh produce, your orders will appear here.</p>
    </div>
  );
}
